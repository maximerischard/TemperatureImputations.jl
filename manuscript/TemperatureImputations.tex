...
The observation model is twofold.
At stations that provide a full temperature time series, we model each measurement as 
To complete the model specification, we add a mean temperature for each station , and iid normal measurement noise \(\epsilon_i\) with variance \(\sigman^2\):
\begin{equation}
\begin{split}
