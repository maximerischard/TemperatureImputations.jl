...
It is long known that climatological summary statistics based on daily temperature minima and maxima suffer from bias due to the observation time.
We develop a novel approach to addressing this problem, by imputing hourly temperatures
at the location of the measurements, which can then be used directly for summary purposes.
We develop a spatio-temporal Gaussian process model to combine information from the observed minima and maxima, and from nearby stations that record hourly temperatures.
To efficiently impute the hourly temperatures from our model, we develop a novel MCMC algorithm.
