...
In this paper, this was illustrated in section \ref{sec:illustrate_bias},
which showed that the answer to a simple question such as ``what was the average daily maximum temperature in 2015?'' can change by about 1\(\degreeC\) depending on the time of measurement.
A drift in the time of measurement from one year to the next could therefore result in misleading conclusions.
In order to redress this bias, we proposed an imputation strategy that combines the information available from the record of daily temperature extremes,
...
This is precisely the information that is missing from daily measurements of temperature extremes made at research-quality weather stations.
We therefore have both pieces of the puzzle, but they are normally made at separate locations.
To combine them, we specified a spatio-temporal Gaussian process model \eqref{eq:gpmodel} with squared exponential kernels.
...
In this paper, we then developed a method based on Hamiltonian Monte Carlo Markov Chains to draw imputation samples from this predictive distribution that respect the constraints imposed by the measured daily extremes: each day's imputed temperatures must attain but not exceed that day's measured \(\Tn\) and \(\Tx\).
