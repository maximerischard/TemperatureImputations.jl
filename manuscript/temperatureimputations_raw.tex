...
Our model thus has four free parameters, \(\sigma_{\subtime}\), \(\ell_{\subtime}\), \(\ell_{\subspace}\) and \(\sigman\), which we fit by maximizing the marginal likelihood \( \Pr\del{ \Temp \mid {\sigma_{\subtime},\ell_{\subtime},\ell_{\subspace},\sigman} }\) of \(\Temp\), the complete 2015 temperature time series provided at the four Iowa weather stations.
...
\begin{equation}
    \Pr\del{\Temp_\miss \mid \Temp_\obs, \Tn, \Tx} = \frac{
        \Pr\del{\Tn, \Tx \mid \Temp_\obs, \Temp_\miss } 
        \Pr\del{\Temp_\miss \mid \Temp_\obs}
        }{
        \Pr\del{\Tn, \Tx \mid \Temp_\obs}
        }\,.
\end{equation}
...
We then assess whether these models improve the semivariogram and the predictive diagnostics that we presented in \autoref{sec:diagnostics}.
...
\begin{equation}
% \begin{split}
    \concordance_\hour = \log \Pr\del{ \Temp_\miss \! = \! \mu\del{\hour} \mid \Temp_\obs }
    \,\text{, where}\ \,
...
Ideally we would wish to estimate the posterior probability \(\Pr\del{\hour \mid \Temp_\obs,\Tn,\Tx}\), for example by sampling from the joint posterior of \(\Temp_\miss, \hour \mid \Temp_\obs,\)\(\Tn\),\(\Tx\), but this is computationally difficult.
