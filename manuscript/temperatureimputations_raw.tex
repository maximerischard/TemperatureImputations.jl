\section{Introduction}\label{sec:climate_intro}
...
\begin{figure}[tbp]
\centering
\includegraphics[height=0.4\textheight,width=0.99\textwidth,keepaspectratio]{waterloo_triangles.pdf}
\caption{
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.6\textwidth]{Iowa_map.pdf}
\caption{
\label{fig:Iowa_map}
...
\begin{figure}[tbp]
\centering
\includegraphics[height=0.3\textheight,width=0.99\textwidth,keepaspectratio]{waterloo_bias_2x2.pdf}
\caption{
...
% \begin{figure}[tbp]
% \centering
% \includegraphics[height=0.3\textheight,width=0.99\textwidth,keepaspectratio]{waterloo_meanabsdiff.pdf}
...
The methodology relies on models and approximations that may garner some scepticism.
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.99\textwidth]{paper_in_one_figure.pdf}
...
\section{Methods: missing data imputation framework}\label{sec:methods}
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.99\textwidth]{imputations_2x2.pdf}
...
\begin{split}
    ...
    \mu_{\miss \mid \obs} &= \E \del{\Temp_\miss \mid \Temp_\obs} = \Kmo \Soo^{-1} \Temp_\obs \,\text{, and} \\
        % &= \cov\del{\Temp_\miss, \Temp_\obs} \cov\del{\Temp_\obs, \Temp_\obs}^{-1} \Temp_\obs\,, \\
    \Sigma_{\miss \mid \obs} &= \var \del{\Temp_\miss \mid \Temp_\obs} = \Smm - \Kmo \Soo^{-1} \Kmo\trans
        % &= \cov\del{\Temp_\miss,\Temp_\miss} - \cov\del{\Temp_\miss, \Temp_\obs} \cov\del{\Temp_\obs, \Temp_\obs}^{-1} \cov\del{\Temp_\obs, \Temp_\miss}
    \,.
\end{split}
...
\subsection{Imputing by conditioning on extrema with SmoothHMC}
\label{sec:smoothHMC}
...
\begin{equation}
    \prob\del{X \mid \Xmax,\Xmin} 
        % \propto \prob\del{X} \prob\del{\Xmax,\Xmin \mid X} 
...
\begin{figure}[tbp]
\centering
\includegraphics[height=0.3\textheight,width=0.99\textwidth,keepaspectratio]{constraints3d.pdf}
...
\begin{figure}[!tb]
\centering
\includegraphics[width=0.99\textwidth,height=0.55\textheight,keepaspectratio]{toy_quantiles.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.97\textwidth,height=0.7\textheight,keepaspectratio]{toy_joint.pdf}
...
Imputations are obtained in nine day batches for computational reasons, with three days of overlap between adjacent batches so each imputation can be made at least three days away from the batch's edges (see \autoref{appendix:batch_details} for details).
...
\begin{equation}
    \gamma\del{\hvec,r} = 
    % \V\sbr{Y(\xvec+\hvec, t+r) - Y(\xvec,t)}
...
\begin{figure}[!tbp]
\centering
\includegraphics[width=0.99\textwidth]{spatial_variogram.pdf}
...
\begin{equation}
    \label{eq:mse}
    % \mse\del{\error \mid \Temp_\obs,\Tx,\Tn} = 
...
The most salient feature of the empirical semivariogram that is not captured by the \(\kSESE\) covariance is the oscillation with a 24-hour period.
...
\begin{figure}[tbp]
\centering
\includegraphics[height=0.35\textheight,width=0.99\textwidth,keepaspectratio]{imputed_summary_stats.pdf}
...
The yearly mean of the daily minimum and maximum temperatures is perhaps of limited inherent scientific interest.
...
\begin{figure}[t]
    ...
    \includegraphics[width=\textwidth]{mean_TnTx/725080-14740_mean_temp_by_hour.pdf}
    \caption{
    ...
    ~
    \begin{subfigure}[t]{0.37\textwidth}
    \includegraphics[width=\textwidth]{mean_TnTx/722350-3940_mean_temp_by_hour.pdf}
    \caption{
...
The poor coverage of the credible interval at KBDL prompted several improvements to our model, which we now discuss.
...
Given $J$ stations, let us denote the hourly data at station $J$ by $\Tobs^j$,
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.99\textwidth]{stations_map_44.pdf}
...
obtained if the measurements are at 17:00~UTC each day.
For ease of visualization, we subtract the true value of the yearly mean temperature (extracted directly from the withheld hourly data) from the estimates.
We compare this estimate to the mean of all SmoothHMC imputations obtained at the same location.
The darker errors bars span $\pm 2$ posterior standard deviations on either side of the posterior mean,
while the lighter errors bars additionally incorporate the estimated covariance for days
...
\begin{figure}[p]
    \centering
    \includegraphics[width=0.6\textwidth]{infermean_errors.pdf}
    \caption{
        ...
        daily minima and maxima measured at different hours of the day.
    }
\end{figure}

The results show a clear improvement: averaging $\Tn$ and $\Tx$ is significantly biased upwards, as is expected
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.9\textwidth,height=0.4\textheight,keepaspectratio]{measure_hour_example.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics[height=0.4\textheight,width=0.9\textwidth,keepaspectratio]{hr_inference.pdf}
\caption{
